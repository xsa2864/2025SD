...
if (!function_exists('cdnurl')) {
    /**
     * 覆盖式增强：同时支持单路径 & 逗号批量路径
     * 其余正则、cdnurl 配置与官方保持完全一致
     *
     * @param string       $url    单路径 或 逗号拼接的多路径
     * @param bool|string  $domain 是否带域名，或直接传入域名
     * @return string
     */
    function cdnurl($url, $domain = false)
    {
        $regex = "/^((?:[a-z]+:)?\/\/|data:image\/)(.*)/i";
        $cdnurl = \think\Config::get('upload.cdnurl');   // 读取后台配置的CDN域名

        /* ---------- 多条路径 ---------- */
        if (strpos($url, ',') !== false) {
            $arr = array_filter(explode(',', $url));
            $arr = array_map(function ($v) use ($cdnurl, $regex, $domain) {
                // 先走官方相对路径逻辑
                if (is_bool($domain) || stripos($cdnurl, '/') === 0) {
                    $v = preg_match($regex, $v) || ($cdnurl && stripos($v, $cdnurl) === 0)
                        ? $v
                        : $cdnurl . $v;
                }
                // 再拼域名
                if ($domain && !preg_match($regex, $v)) {
                    $prefix = is_bool($domain) ? request()->domain() : rtrim($domain, '/');
                    $v = $prefix . '/' . ltrim($v, '/');
                }
                return $v;
            }, $arr);
        ...
        /* ---------- 单条路径：完全兼容官方原版 ---------- */
        if (is_bool($domain) || stripos($cdnurl, '/') === 0) {
            $url = preg_match($regex, $url) || ($cdnurl && stripos($url, $cdnurl) === 0)
                ? $url
                : $cdnurl . $url;
        }
